...
1. **运行 claims 解析器 + 词汇表管理**：
   ...
   词汇表从项目实际使用的谓词中自动提取——每个项目的词汇表不同，硬编码推荐谓词无意义。
   见 `templates/claims-format-spec.md` 和 `templates/claims-vocab.md`。
...
2. 基于解析器输出进行分级检查：
   ...
   **WARN（词汇表）**：
   - 词汇表尚未生成 → **INFO**："运行 --gen-vocab 自动生成项目词汇表"
   - `--vocab` 检出未在项目词汇表中注册的**谓词** → **WARN**："谓词未注册"（可在页面中使用新谓词后重新 --gen-vocab）
   - `--vocab` 检出未在项目词汇表中注册的**实体名** → **WARN**："实体名未注册"（可能是拼写不一致，如 "TAU" vs "TAU0"）
   - 词汇表覆盖谓词和实体名——两者都是 Datalog 三元组匹配的前提
